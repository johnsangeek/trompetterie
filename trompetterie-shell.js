...
(()=>{
  ...
  const root=$("#drawerRoot"),scale=$("#drawerScale"),summary=$("#drawerSummary");
  ...
  function updateSummary(){const mode={hero:"Héros",instrument:"Instrument",partition:"Partition"}[selectedMode()];summary.textContent=selectedSource()==="scale"?`${noteLabels[Number(root.value)]} trompette · ${scaleLabels[scale.value]} · ${mode}`:`Premiers pas · ${mode}`;}
  ...
  [root,scale,...document.querySelectorAll('input[name="drawerMode"]')].forEach(input=>input.addEventListener("change",updateSummary));
  $("#drawerImportMidi").addEventListener("click",()=>{$("#midiInput")?.click()});
  $("#drawerPlay").addEventListener("click",()=>{
    const detail={source:selectedSource(),mode:selectedMode(),root:Number(root.value),scale:scale.value,music:document.querySelector('input[name="drawerMusic"]:checked')?.value||"demo"};
