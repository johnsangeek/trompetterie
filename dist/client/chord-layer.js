...
function renderResults(){const wrap=$("layerSequence");wrap.innerHTML="";if(!state.generated.length){wrap.innerHTML='<div class="layer-empty">Aucune réponse générée.</div>';return}const varied=state.generated.filter(item=>item.phraseLabel!=="Exposition").length;$("phraseSummary").textContent=`Forme détectée : ${state.generated.length} mesures. La reprise utilise ${varied} variations, avec une montée de tension sur l’avant-dernière mesure et une finale distincte.`;state.generated.forEach((item,index)=>{const row=document.createElement("article");row.className=`layer-row ${item.isEnding?"ending":""}`;row.innerHTML=`<span class="layer-row-index">${String(index+1).padStart(2,"0")}</span>${chordCell("base","Socle",item.base.notes,item.chord)}${chordCell("original","Original",item.original.notes)}${chordCell("generated",item.phraseLabel,item.notes,item.styled)}`;row.querySelector(".generated").onclick=()=>playNotes(item.notes);wrap.appendChild(row)});$("resultPanel").hidden=false}
