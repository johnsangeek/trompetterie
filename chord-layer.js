...
const state={master:null,overlay:null,style:"jazz",audacity:2,generated:[],audio:null};
...
function movementCost(from,to){if(!from)return 0;const count=Math.min(from.length,to.length);let cost=Math.abs(from.length-to.length)*4;for(let i=0;i<count;i++)cost+=Math.abs(from[i]-to[i]);return cost}
function createSuggestions(master,overlay,style,audacity){
  const cycleStart=master.groups[0]?.start||0,cycleEnd=Math.max(...master.groups.map(group=>group.end),cycleStart+master.division),cycleLength=Math.max(1,cycleEnd-cycleStart);let previous=null;return overlay.groups.map((source,index)=>{const wrappedStart=cycleStart+((source.start-cycleStart)%cycleLength+cycleLength)%cycleLength,active=master.groups.filter(group=>group.start<=wrappedStart&&group.end>wrappedStart).sort((a,b)=>b.start-a.start)[0],nearest=master.groups.reduce((best,group)=>!best||Math.abs(group.start-wrappedStart)<Math.abs(best.start-wrappedStart)?group:best,null),base=active||nearest;if(!base)return null;const found=detectChord(base.notes),quality=styleQuality(found,style,audacity),candidates=voicingCandidates(found.root,quality,audacity),target=68+(index%4===1?3:index%4===3?-2:0)+(audacity===3?(index%2?3:-2):0);candidates.sort((a,b)=>(movementCost(previous,a)+Math.abs(a.reduce((s,n)=>s+n,0)/a.length-target)*1.6)-(movementCost(previous,b)+Math.abs(b.reduce((s,n)=>s+n,0)/b.length-target)*1.6));const notes=candidates[0];previous=notes;return{start:source.start,end:source.end,velocity:source.velocity,base,original:source,notes,chord:{root:found.root,quality},styled:{root:found.root,quality}}}).filter(Boolean);
}
...
async function loadFile(kind,file){if(!file)return;const status=$(`${kind}Status`);status.textContent="Analyse…";try{const parsed=parseMidi(await file.arrayBuffer());state[kind]=parsed;fileStatus(kind,file,parsed);renderAnalysis();state.generated=[];$("resultPanel").hidden=true}catch(error){status.textContent=error.message;toast("Impossible d’analyser ce MIDI")}}
function chordCell(className,label,notes){const found=detectChord(notes);return `<div class="layer-chord ${className}"><small>${label}</small><strong>${chordName(found)}</strong><p>${notes.map(midiName).join(" · ")}</p></div>`}
function renderResults(){const wrap=$("layerSequence");wrap.innerHTML="";if(!state.generated.length){wrap.innerHTML='<div class="layer-empty">Aucune réponse générée.</div>';return}state.generated.forEach((item,index)=>{const row=document.createElement("article");row.className="layer-row";row.innerHTML=`<span class="layer-row-index">${String(index+1).padStart(2,"0")}</span>${chordCell("base","Socle",item.base.notes)}${chordCell("original","Original",item.original.notes)}${chordCell("generated","Réponse",item.notes)}`;row.querySelector(".generated").onclick=()=>playNotes(item.notes);wrap.appendChild(row)});$("resultPanel").hidden=false}
function generate(){if(!state.master||!state.overlay)return;state.generated=createSuggestions(state.master,state.overlay,state.style,state.audacity);renderResults();$("resultPanel").scrollIntoView({behavior:"smooth",block:"start"});toast(`${state.generated.length} voicings reliés créés`)}
...
if(typeof module!=="undefined")module.exports={parseMidi,detectChord,estimateKey,createSuggestions,midiBytes,chordName,midiName};
